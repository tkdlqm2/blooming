 ...
 * @dev ERC20 토큰 + 거버넌스 기능 + 포인트 교환 기능
 */
contract GovernanceToken is ERC20, ERC20Votes, ERC20Permit, Ownable, Pausable, ReentrancyGuard {
    ...
    /**
     * @dev 포인트를 토큰으로 교환
     * @param user 교환받을 사용자 주소
     * @param points 교환할 포인트 수량
     */
...
contract GovernanceContract is Ownable, ReentrancyGuard, Pausable {
    ...
    function propose(
        ...
        // 제안 임계값 확인
        if (governanceToken.getPastVotes(msg.sender, block.number - 1) < PROPOSAL_THRESHOLD) {
        ...
        newProposal.startBlock = block.number + (VOTING_DELAY / 12); // 블록 단위 변환
