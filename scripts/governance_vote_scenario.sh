...
# 9. 투표 진행
log_info "거버넌스 시작 시간이 약간의 블록 갯수 오차가 있기에 안전적인 테스트를 위해서는 1분 딜레이!"
log_info "1분 후 투표를 시작합니다..."
...
if [ "$PROPOSAL_STATUS" = "VOTING" ]; then
    ...
    for i in $(seq 1 $YES_VOTES); do
        if [ $i -le $USER_COUNT ]; then
            ...
            # JSON 파일 생성 (UTF-8 인코딩)
            VOTE_JSON=$(cat <<EOF
{
    "voterId": "$USER_ID",
    "voteType": "YES",
    "reason": "User $i YES vote",
...
EOF
            ...
            # 임시 JSON 파일 생성
            echo "$VOTE_JSON" > /tmp/vote_${i}.json
